...
// WiFi配置参数
#define EXAMPLE_ESP_WIFI_SSID      CONFIG_WIFI_SSID        // WiFi名称
#define EXAMPLE_ESP_WIFI_PASS      CONFIG_WIFI_PASSWORD    // WiFi密码
...
esp_err_t wifi_init_softap(void)
{
    ...
    uint8_t random_channel = 1 + (rand() % 13); // 生成1-13之间的随机信道
    wifi_config_t wifi_config = {
        .ap = {
            .ssid = EXAMPLE_ESP_WIFI_SSID,
            .ssid_len = strlen(EXAMPLE_ESP_WIFI_SSID),
            .channel = random_channel,  // 使用随机信道
    ...
    ESP_LOGI(TAG, "WiFi初始化完成. SSID:%s 密码:%s 信道:%d",
             EXAMPLE_ESP_WIFI_SSID, EXAMPLE_ESP_WIFI_PASS, random_channel);
    return ESP_OK;
}
